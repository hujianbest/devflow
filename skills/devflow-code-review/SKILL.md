...
- 写 / 修代码 → `devflow-tdd-implementation`
- 评审测试有效性 → `devflow-test-review`
- 阶段不清 / 证据冲突 → `devflow-router`

## Entry Gate

- `devflow-test-review` 已给出 `通过` verdict（未经 test-review → 不得进入，Hard Stop #9）
- 由**独立 reviewer subagent** 执行；不改生产代码

## 硬性门禁

- `devflow-test-review` 未通过前不得进入本 review
- `devflow-code-review` 通过前不得进入 `devflow-completion-gate`
- reviewer **不**修改代码 / 测试 / 设计
- reviewer **不**替模块架构师 / 开发负责人拍板
- reviewer **不**重新审组件级架构决策（那是 `devflow-component-design-review` 的职责）；只做 conformance check
- reviewer **不**返回多个候选下一步
- 代码修改若实质上改写组件接口 / 依赖 / 状态机 / SOA 边界 → 强制 `阻塞`(workflow)，下一步 `devflow-router`
...
- `通过` → `devflow-completion-gate`
- `需修改` / `阻塞`(内容，含 critical C/C++ 风险) → `devflow-tdd-implementation`
...
- 「测试通过」当作「代码正确」
- 修改 / 删除既有行为时只看新结果，未核对保留行为、批准破坏或删除语义
- 忽略未解释的 critical 静态分析 / 编译告警
- 评审中改代码（reviewer 不是 author）
- 重新审组件级架构（应该回 `devflow-component-design-review`）
...
评审代码时，检查 refactor notes 是否区分 GREEN implementation 与 REFACTOR cleanup，是否列出 touched files/functions，是否解释 cleanup 为什么仍在当前 task 边界内，并列出 cleanup 后重新运行的 verification。跨模块 cleanup 或组件边界变化必须路由到 `devflow-router`。
