...
- `features/<id>/requirement.md` 已存在且可读
- 由**独立 reviewer subagent** 执行（作者不自审）；reviewer 不修改 `requirement.md`、不补内容
- 已知 `Workflow Profile`（决定通过后的下一步）
...
- `通过`：requirement-analysis → `devflow-component-design`（触发组件设计修订）或 `devflow-finalize`（仅澄清）；实现 → `devflow-component-design`（component-impact）或 `devflow-ar-design`（其余）
- `需修改` / `阻塞`(内容) → `devflow-specify`
...
- 忽略 IR / SR / AR 追溯断裂
- 忽略 `modify` / `remove` row 缺旧行为基线或回归 / 删除语义
...
规格评审常见的偷懒话术与反驳。命中任意一条 → 停下。

| 话术 | 反驳 |
|---|---|
| 「我刚扫了一眼，整体没问题，直接 `通过`」 | 必须按 6 维度 rubric 评分，不允许自由阅读式评审 |
| 「缺业务事实，我帮它补一下吧」 | reviewer **不**补业务事实 / 优先级 / 阈值。分类为 `USER-INPUT`，由父会话上抛需求负责人 |
| 「需求负责人没签字，但内容看起来 OK，给 `通过`」 | 必须 `通过` + `needs_human_confirmation=true`，不能省略人工确认标记 |
