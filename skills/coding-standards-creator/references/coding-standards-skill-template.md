...
> 配套 `coding-standards-creator`。拷贝后替换尖括号占位；各部分的写法要求见 `coding-standards-skill-contract.md`。正式产出不得残留占位符与本说明。

````markdown
---
name: <language>-coding-standards
description: 在编写、修改或评审 <语言> 代码（<源文件后缀/测试/构建脚本>）时使用。提供 <主题1>、<主题2>、<主题3> 的具体规则与正反例。<相邻语言> 规则见 <相邻技能名>。
---
