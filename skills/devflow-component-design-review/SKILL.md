...
- `features/<id>/component-design-draft.md` 存在且可读
- 由**独立 reviewer subagent** 执行；不修改设计草稿、不替模块架构师拍板
...
| CD7 AR Design Constraints & Cross-Component Impact | 「对 AR 实现设计的约束」章节存在；跨组件影响显式列出；`modify` / `remove` 的兼容 / 迁移 / 废弃策略已消费 |
...
按 verdict（详见 `references/reviewer-dispatch-protocol.md`）：

- `通过`：requirement-analysis → `devflow-finalize`（analysis closeout）；实现(component-impact) → `devflow-ar-design`
- `需修改` / `阻塞`(内容) → `devflow-component-design`
- `阻塞`(workflow) → `reroute=true`（→ `devflow-router`）
...
- 因模板未补齐就给 `通过`
- 跨组件影响未列出却给 `通过`
- 修改 / 删除既有组件行为但缺旧行为基线、兼容策略、迁移 / 废弃路径却给 `通过`
- 因「实现简单」就放过状态机覆盖不全
- findings 无分类

## 反向理由化（Common Rationalizations）

组件设计评审常见的偷懒话术与反驳。命中任意一条 → 停下。

| 话术 | 反驳 |
|---|---|
| 「我顺手把设计章节补一下，这样能直接 `通过`」 | reviewer 是 gate，不是 author。修改被评审工件被禁止，必须返回 finding |
| 「模块架构师没签字，但内容看起来 OK，给 `通过`」 | 必须 `通过` + `needs_human_confirmation=true`；架构师 sign-off 是硬性 USER-INPUT |
| 「Design Options 形式上有了就算过」 | 必须检查是否用 `Single obvious option` 掩盖了真实决策；裸跳过 → `需修改` |
| 「ABI 兼容性看代码就知道，不必写」 | 接口签名 / 错误码 / 语义有变 → 必须显式分析 ABI / API 兼容性 |
| 「跨组件协调先放过，下游团队应该没问题」 | 跨组件影响必须有明确协调路径 / 下游 owner，否则 `需修改` |
| 「删除旧接口路径只要新接口设计好了就行」 | `remove` 必须审查旧行为基线、已知消费者、删除后语义和迁移 / 废弃策略 |
...
| `modify` / `remove` 未审查兼容 / 迁移 / 废弃策略 | 加 important/critical finding，要求回 `devflow-component-design` |
