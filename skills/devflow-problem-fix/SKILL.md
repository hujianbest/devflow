...
- `features/DTS<id>-<slug>/progress.md`：`Current Stage = devflow-problem-fix`、`Pending Reviews And Gates = test-review, code-review, completion-gate`、`Next Action Or Recommended Skill = <步骤 6 回流节点>`
...
| 「hotfix 不用 `devflow-test-review` / `devflow-code-review`」 | hotfix profile 仅压缩文档量，**不**跳门禁。test-review / code-review / completion-gate 一项不能少 |
