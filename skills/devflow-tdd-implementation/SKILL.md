...
本 skill 不写 AR 设计、不改变 AR 范围、不自审测试、不自审代码；这些职责分别属于 `devflow-ar-design`、`devflow-test-review`、`devflow-code-review`。
...
- `devflow-test-review` 或 `devflow-code-review` 对实现工作返回 `needs changes`。
...
- 测试需要独立有效性审查：使用 `devflow-test-review`。
- 代码需要独立评审：使用 `devflow-code-review`。

## Entry Gate

- AR 实现设计已通过 `devflow-ar-design-review` 且**含测试设计章节**（缺章节 → 回 `devflow-ar-design`，Hard Stop #6）
- 内部 task queue preflight 必须产出完整 tasks 与唯一 `Current Active Task`（否则 `reroute=true`，Hard Stop #7/#8）
- 本 skill 是**唯一的 implementer subagent 派发者**（`references/devflow-conventions.md` §10）
...
- 不得自称测试有效或代码质量合格；下一步派发 `devflow-test-review`。
...
实现 `Current Active Task` 时默认使用 fresh implementer subagent。只有极小、低风险、单文件且 controller 已经持有完整上下文的修改，才允许 controller-direct；一旦需要探索多文件代码、测试 harness、构建配置或静态分析输出，就必须派发 implementer subagent。controller 仍负责 routing、task queue 状态、evidence 路径和最终 handoff。

不要让 implementer subagent 读取完整计划、历史聊天或大范围仓库上下文。先构建 context pack，把必要事实直接放进派发 prompt；如果 context pack 无法小而精确，说明 task 还不够可执行，应先拆分、收敛或回到上游设计。

### Implementer Context Pack

派发 implementer subagent 前，基于已批准产物写出或组装以下 context pack。Pack 是可执行契约，不是资料倾倒；每一项都应只包含当前 task 必要事实、路径或短摘录。
...
- Requirement Rows:
- Change Type / Existing Behavior Baseline:
- AR Design Anchors:
...
- `DONE`: 实现与 fresh evidence 已就绪；controller 记录报告并派发 `devflow-test-review`。
- `DONE_WITH_CONCERNS`: 实现已完成但 subagent 标出不确定点；controller 必须先分类 concerns。若 concern 影响 acceptance、测试设计、组件边界或证据可信度，先处理或回上游；若只是后续清理观察，记录到 `implementation-log.md` 后再派发 `devflow-test-review`。
- `NEEDS_CONTEXT`: controller 补充缺失上下文，并用更收敛的 context pack 重新派发；不得把完整聊天、完整计划或全仓库上下文补给 subagent。
- `BLOCKED`: controller 判断路由到 `devflow-ar-design`、`devflow-router`，或继续拆分 task。若 blocker 是 scope、profile、组件边界或外部依赖决策，必须交回 `devflow-router`。
...
每个 task 必须包含 Task ID、Goal、Acceptance、Files、Covers Requirement、Change Type / Existing Behavior Baseline、AR Design Anchor、Test Design Case IDs、Dependencies、Verify、Definition of Done、Expected Evidence Paths、notes/assumptions。
...
- TR5 Traceability：task 映射到 requirement row、Change Type / Existing Behavior Baseline、AR design 和 test case。
...
为 `Current Active Task` 创建 Implementer Context Pack。派发前把路径或摘要记录到 `task-board.md`。若 pack 无法做到小而精确，说明 task 太宽，应先拆分或收敛；不得用“让 subagent 自己探索”来弥补不完整 pack。

### 5. 派发 Implementer Subagent

派发新的 implementer subagent，只提供 context pack、允许的文件范围和预期 evidence 路径。implementer 在受限上下文中执行下面步骤 6-10。若 subagent 提问，在 controller session 中回答，并用澄清后的 context pack 重新派发；不要让同一个 subagent 继续扩大探索范围。
...
只实现 `Current Active Task` 对应 Test Design Case IDs 的测试。名称或注释中保留 Task ID 与 Case ID 锚点。`modify` task 必须包含对应 regression 断言或批准破坏行为断言；`remove` task 必须包含旧入口删除后的可观察语义断言。
...
更新 progress 字段：Current Stage、Task Plan Path、Task Board Path、Current Active Task、Pending Reviews And Gates、`Next Action Or Recommended Skill = devflow-test-review`。同步 `task-board.md` 的 dispatch status、context pack location/summary、implementation report、evidence paths 和 blocked reason（如有）。

派发独立 reviewer subagent 执行 `devflow-test-review`；不要内联执行测试评审。
...
- 下一步：`devflow-test-review`。

## Exit Handoff

- 成功 → `devflow-test-review`
- 需改设计 → `devflow-ar-design`；task queue 无法唯一确定或属 route/profile/scope blocker → `reroute=true`（→ `devflow-router`）
- 依据 `references/devflow-conventions.md` §8 转移表

## 风险信号

- task queue preflight 通过前就开始实现。
- 派发 implementer subagent 时给了大段聊天历史，而不是精选 context pack。
- `modify` / `remove` task 没把 Existing Behavior / Baseline 放进 context pack，导致实现只按新目标行为写。
...
- [ ] `modify` / `remove` 的 baseline delta 已进入 task、context pack、测试断言和 evidence
...
- [ ] `traceability.md` 已更新
- [ ] `progress.md` 已路由到 `devflow-test-review`
