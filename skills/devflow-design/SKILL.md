...
输入是已确认的 `spec.md`，输出是 `features/<id>/design.md`（模板见 `references/design-template.md`）。设计的深度跟随变更的影响面：纯内部修改写清楚改哪些模块和测试设计即可；触及对外接口、组件依赖、状态机时，必须把契约和兼容策略写到可冷读。

**设计的第一律：简单性。** 满足当前规格的最少结构就是好结构。每多一层间接、一个抽象、一个配置项，都要付出理解、测试和演进的复利成本。本文所有原则最终都服务于这一条。

## 工作流

1. **读 spec**，列出本变更触碰的既有模块与新增职责。
2. **划分模块职责**（见下文 §职责与边界）。
3. **设计接口契约与错误模型**（见 §接口契约、§错误模型）。
4. **记录方案取舍**：有真实可选方案时写 2-3 个选项的对比；只有一个合理方案时写明其他方案为什么不成立（见 §方案取舍）。
5. **写测试设计**：把 spec 的每条验收标准映射成测试用例表（见 §测试设计）。
6. **自检**（文末清单）后交人审查；人确认前不进入实现。

实现中发现设计有误：停下、回来改 design.md、重新确认，不在代码里悄悄偏离。
...
- [ ] 适用的语言规范（`c-coding-standards`/`cpp-coding-standards`）与领域约束（`embedded-development`/`automotive-development`）已读取并体现在契约里

## 支撑参考

| 文件 | 用途 |
|---|---|
| `references/design-template.md` | design.md 模板 |
