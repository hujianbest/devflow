---
documentType: canonical-design
component: <组件名称>
baselineStatus: draft | baseline-ready
baselineRevision: <不可变仓库修订>
...
- 本模板用于 `devflow-init` 建立既有组件设计基线，或由首个新组件变更生成 `<component-root>/specs/design.md`。后续 `devflow-design` 只写 `specs/changes/ARXXX-<topic>/delta-design.md`，由 `devflow-ship` 经独立复核和人工确认后同步本文件。
- 正式交付件必须删除模板说明、示例业务内容和任何占位符；不得残留 `AI提示`、`TBD`、`{DATE}` 等模板痕迹。
- 本文档采用团队组件设计作业结构：修订记录、术语、概述、输入、组件详细设计、关键功能设计、评审纪要、成本评估均为必填骨架。
- 本文档是 `delta-design.md` 的组件基线。下游设计增量必须引用功能编号、接口契约、软件单元、功能场景时序图和测试项，不得重新定义组件级架构。
- `baselineStatus` 只有在来源追溯完整、独立评审通过且人工确认后才能写为 `baseline-ready`；高质量设计判断由 R2 和规范同步评审准则执行，不在本模板中重复设置章节。
...
### 5.0 方案选择（Design Options，必要）

在写完整组件详细设计前记录组件级候选方案、方案权衡、推荐项和模块架构师确认状态。若只有一个显然方案，写 `单一明显方案` 并说明为什么不展开多方案。

| 方案 | 职责 / 非职责变化 | 接口 / 依赖 / 状态机影响 | 并发 / 实时 / 资源风险 | 跨组件协调 | 成本 / 取舍 | 结论 |
|---|---|---|---|---|---|---|
| A |  |  |  |  |  | 推荐 / 拒绝 |
| B |  |  |  |  |  | 推荐 / 拒绝 |
...
建议使用 PlantUML 部署图完成代码结构模型展示，体现 CMake / 源码目录组织、分层和依赖方向。
...
明确组件并发场景约束、多任务 / 定时器 / 线程 / 协程模型，并说明自旋锁（spinlock） / 原子操作 / 互斥锁（mutex） / 序列化消息等保护机制及开销。
...
#### 6.1.1 对外服务（Service）接口

| 接口名 | 描述 | 方法 | 参数 | 返回值 / 错误码 | 是否支持并发 | 并发约束 / 保护机制 | 兼容性策略 |
|---|---|---|---|---|---|---|---|
|  |  |  |  |  | 是 / 否 |  |  |

#### 6.1.2 对外主题（Topic）接口（可选）

| 主题（Topic）名称 | 描述 | 载荷 | 是否支持并发 | 并发约束 / 保护机制 |
|---|---|---|---|---|
|  |  |  | 是 / 否 |  |

#### 6.1.3 对外 API 接口

| API 名称 | 描述 | 参数 | 返回值 / 错误码 | 是否支持并发 | 并发约束 / 保护机制 |
|---|---|---|---|---|---|
|  |  |  |  | 是 / 否 |  |
...
| AR 编号 | AR 描述 | 兼容性影响 | 归档的设计增量文档地址 |
|---|---|---|---|
|  |  |  | `specs/archive/YYYY-MM-DD-ARXXX-<topic>/delta-design.md` |
...
##### 6.3.1.2 示例跟踪类（ExampleTrack）描述

| 序号 | 函数类型 | 函数名称 | 函数功能 | 输入 / 输出 | 错误处理 |
|---|---|---|---|---|---|
| 1 | 外部接口 / 内部接口 |  |  |  |  |

##### 6.3.1.3 示例管理类（ExampleMgr）描述
