---
name: devflow-ar-design-review
description: 当 devflow-ar-design 产出的 ar-design-draft.md 需要独立评审结论时使用；也用于派发评审子代理审查 AR 级代码设计及其内嵌测试设计章节，或修订后重跑 AR 设计评审。不用于编写或修订 AR 设计、组件设计评审，或阶段和路由混乱。
---
...
- `devflow-ar-design` 已产出 ar-design-draft.md，需正式 verdict
- reviewer subagent 被派发执行 AR 设计评审
- 用户明确要求「review 这份 AR 设计 / 评审 AR 实现设计」

不适用 → 改用：

- 缺草稿或仅需继续写 → `devflow-ar-design`
- 组件级设计评审 → `devflow-component-design-review`
- 阶段不清 / 证据冲突 → `devflow-router`

## Entry Gate

- `features/<id>/ar-design-draft.md` 存在且**含测试设计章节**（缺章节 → 返回 `devflow-ar-design`，不放行）
- 仅实现子街区；由**独立 reviewer subagent** 执行；不修改设计草稿
...
- `通过` → `devflow-tdd-implementation`（含 task queue preflight）
- `需修改` / `阻塞`(内容) → `devflow-ar-design`
...
| 修改 / 删除既有行为只测新目标结果 | 补 regression / removal 用例，并回指 requirement row 的 Existing Behavior / Baseline |
