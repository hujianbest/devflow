...
除非 `AGENTS.md` 覆盖路径，否则写入 `features/<id>/reviews/ar-design-review.md`。

### Review 边界

评审 AR 代码层设计、component-design 一致性、内嵌测试设计、追溯和实现风险。若设计改变组件接口 / 依赖 / 状态机，返回 workflow-blocked 并路由到 `devflow-router`。
## 支撑参考
