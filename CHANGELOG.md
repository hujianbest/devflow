...
- Added `writing-readable-doc`, a Chinese-first writing overlay loaded *before* an agent produces or edits any delivery document (`srs.md`, both deltas, canonical spec/design, `closeout.md`, `docs/learnings/`). `devflow-init`, `devflow-specify`, `devflow-design`, `devflow-fix`, `devflow-ship`, and `devflow-learn` load it as soon as they start writing prose; reviewers use it as R1/R2/sync readability criteria. It defines judgement-first drafting rules, a five-question cold-read test, a document-smell catalog, Chinese engineering writing rules, and a meaning-preserving rewrite protocol that keeps stable IDs, thresholds, EARS statements, Given/When/Then acceptance, QAS elements, and delta base excerpts verbatim. Vague wording is routed back to `devflow-specify` / `devflow-design` as a content defect instead of being paraphrased away. It adds no gate, no file, and no `change.json` field.
...
- Skills, commands, and agents now load skills by name instead of by repository path, and no skill reaches into a sibling skill through `../<skill>/`. References to a file inside another skill are written as "skill name + file within that skill". `validate_devflow.py` enforces both rules plus skill-name uniqueness across skill roots, and the implementer Context Pack field is now `required_skills` (names) rather than `required_skill_files` (paths).
