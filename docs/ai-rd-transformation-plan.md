...
| SR→AR 分解 + 功能设计 | SE 写功能设计说明书 | AI 起草功能设计：接口契约、状态机、错误模型、异常路径、与其他 SR 的依赖；SE 评审修订 | L2 | R2：功能设计评审，SE 签字 |
| 需求串讲 | 全员会议两小时 | 取消大串讲。AR 包（spec + 追溯 + 上下文）机器可读，开发直接向 AI 提问；只对有争议的 AR 开小会 | L3 保留例外会 | 问答记录回流知识库 |

SE 的角色从"文档写手"变成"需求资产的运营者"：判断分解是否合理、裁决 AI 标出的冲突、维护需求模式库。一个 SE 能管的 SR 数量会显著上升，瓶颈从"写"移到"判断"。

### 2.3 开发段：实现设计先行，代码由测试证明
...
| 步骤 | 传统做法 | AI 产线做法 | 分级 | 门禁 |
|------|----------|-------------|------|------|
| AR 理解 | 听串讲 + 读功能设计说明书 | AR spec 机器可读，AI 答疑；AI 生成该 AR 的影响面分析（触及哪些模块、接口、消费者） | L1（问答）/ L2（影响面） | 影响面分析纳入评审材料 |
| 实现设计 | 写实现设计说明书 | AI 起草实现设计：时序图（PlantUML 文本化）、类图、接口契约、错误模型、UT 用例清单；开发修订。安全相关决策（喂狗时机、安全状态、DTC 策略）自动识别为上抛项，不由 AI 拍板 | L2，安全项 L3 | R3：设计评审，关键设计人签 |
...
这里的关键转变有两个。一是实现设计说明书从"交付物"变成"活 spec"：它和代码同库管理、同步演进，变更历史可追溯，下一个改这块代码的人和 AI 读的是当前真相而不是半年前的快照。二是代码审查的矛盾被解开：AI 辅助下代码量会上一个量级，人逐行审必然崩盘，所以改为"AI 全量初审 + 人聚焦关键项"，人审的对象从代码行变成 findings 和设计符合性。
...
| 测试规格 | 手写测试规格 | AI 起草测试规格：覆盖维度、测试层级（SIL/HIL/整车）、边界与异常场景；质量工程师评审覆盖策略 | L2 | T1：规格评审 |
...
G0/R1/R2/G7 是人必须到场的签字点（L3 性质），R3/R4/T1/T2 是"AI 全量检查 + 人聚焦例外"。门禁记录（谁、何时、基于什么版本、审了什么、结论是什么）自动落盘，这本身就是 ASPICE 审计需要的证据。
...
| 功能设计 | 功能设计说明书（SR→AR） | 每条 AR 含接口契约、状态机、错误模型、异常路径、依赖清单、验收标准；跨 SR 影响显式标注 | 起草 + 影响面分析 | SE | 功能设计说明书 |
...
本仓库（DevFlow）的技能包是本方案"流程线"和"知识线"的现成载体：`devflow-specify`/`devflow-design`/`devflow-tdd`/`devflow-review`/`devflow-ship`/`devflow-fix` 对应开发段的阶段流程与独立评审门禁；`devflow-clean-code` 和各语言编码规范、各 `*-development` 领域技能（含 `automotive-development`）对应质量叠加约束；`coding-standards-creator` 对应知识萃取进技能包的标准动作。变革落地时，第 2.3 节开发段流程可直接按现有技能执行，第 3 节文档要求与技能内工件契约（srs.md、delta-design.md、traceability.md、reviews/）对齐，第 4 节知识飞轮的"入库"环节即以技能 PR 的形式回流本仓库。
