...
### 4.6 三位一体：产线、知识工程与 Skills 工程的关系

第 1 节说过三条主线同时建设，这里把三者的咬合关系说透。产线（全流程工作流）是骨架，知识工程与 Skills 工程是两条供给线，三者构成一个闭环：

```mermaid
flowchart TB
    SK[Skills 工程 · 技能包<br/>流程技能 / 质量叠加技能 / 领域技能] -->|操作规程注入<br/>告诉 AI 怎么干活| LINE[AI 产线 · 全流程工作流<br/>IR→SR→AR→规格→设计→代码→测试→验收]
    KN[知识工程 · 知识库<br/>需求模式库 / 分解范例 / 缺陷模式库 / 风险地图] -->|上下文供给 RAG<br/>告诉 AI 知道什么| LINE
    LINE -->|经验回流<br/>评审意见 · 根因 · 问答 · 上抛决策| KN
    LINE -->|红线与正反例<br/>技能 PR| SK
    KN <-.->|重要知识双形态| SK
```

- **Skills 工程供给"操作规程"**。技能包告诉 agent 怎么干活：阶段规程、评审 rubric、门禁标准、安全红线。载体即本仓库 skills/：specify、design、tdd、review、ship、fix 六个流程技能，clean-code 与各语言编码规范的质量叠加技能，automotive-development 等领域技能。
- **知识工程供给"上下文"**。知识库告诉 agent 这个组织知道什么：需求模式库、分解范例、缺陷模式库、风险地图，各节点 agent 工作时经 RAG 拉取，消费留痕。
- **每个节点既消费也回流**。消费：操作规程（技能）+ 上下文（知识）同时注入。回流：评审意见、问题单根因、AI 答疑问答、上抛决策结论萃取成模式卡进知识库；修订中沉淀的红线与正反例以技能 PR 进技能包。
- **重要知识双形态**。模式卡供人读，技能红线供 AI 强制执行；重要的知识两种形态都要有——只进知识库的红线靠 agent 自觉，只进技能的规则人读不到全貌。
- **Skills 自进化**。技能不是写完就结束：每个技能带评测集，靠"评测 → 改写 → 回归"循环持续变好（见 4.9）；技能 PR 触发回归评测，生产端一次通过率低于基线自动告警。知识侧对应的是度量汰旧：每条知识有 owner 和适用期，零命中条目定期淘汰。

一句话总结：产线每跑一轮，知识和技能就被"用一遍、喂一遍"——经验不流失，AI 越用越聪明。

### 4.7 知识工程落地：KCS 双循环方法论与管理结构

第 4.3 节的飞轮回答了"知识怎么转起来"，这一节回答"用什么方法论、谁来管、按什么标准管"。

**方法论选型：KCS 双循环**。KCS（Knowledge-Centered Success，原 Knowledge-Centered Service）是 Consortium for Service Innovation 维护的业界标准知识运营方法论，在服务与知识密集型组织验证三十余年；2026 年更名并明确其定位：结构化、可信的知识是 AI 成功的前提——agent 的效果上限，就是它读到的知识的上限。这与第 4.1 节"工具能买到、模型会升级，拉开差距的是私有知识"的判断一致。KCS 的双循环与本方案飞轮严格同构：

| KCS | 内容 | 本方案飞轮映射 |
|-----|------|----------------|
| Solve Loop（日常，工作中沉淀） | 复用 → 改进 → 捕捉 → 结构化，复用优先 | 沉淀、入库、消费：生产即沉淀，消费留痕 |
| Evolve Loop（迭代，运营中提升） | 内容健康度、知识域分析（KDE 主导）、持续优化 | 萃取、度量反馈：AI 聚类 + 人审核，指标定方向 |

KCS 的三条核心技法直接采用：reuse is review（每次复用都是一次评审）、flag it or fix it（发现知识有问题，当场修或标记，不绕走）、集体所有制（知识质量是所有使用者的责任，不是知识工程师一个人的）。理论底座用 SECI（隐性知识 → 显性知识的转化）解释专家经验萃取；治理对标 ISO 30401 知识管理体系，保证审计就绪。冷启动仍按第 4.4 节三来源并行：存量逆向入库、试点边跑边沉淀、专家访谈萃取。

**管理结构：三层角色 + 条目元数据标准 + 双环节奏**。

| 角色 | 职责 | 人员设置 |
|------|------|----------|
| 知识治理委员会 | 失效下架、规范改版触发的关联复审、指标复盘 | 跨 squad 集中决策（架构与领域委员会兼任） |
| 知识域专家（KDE） | 萃取裁决、模式卡签发、知识域分析 | 资深 SE / 开发 / 测试兼任 |
| 知识工程师 | 知识库与技能包运营、技能 PR 处置、度量面板 | 0.5–1 人，2–3 个 squad 共享（见第 5.2 节） |

每条知识进库必须带齐五项元数据：owner、适用期、置信度、适用范围、正反例。owner 对这条知识的质量负责到底。运营节奏上，日常走 Solve（评审意见、根因、问答按轻量模板落盘，消费留痕），每个迭代走 Evolve（聚类萃取评审入库、失效条目清理、度量复盘决定下一迭代萃取方向）。

### 4.8 Skills 工程：分层管理（组织维度）

技能包是团队核心资产（第 2.6 节已明确必须自有），资产就要回答"谁建设、谁评审、谁负责"。参照 Anthropic Agent Skills 的工程实践（渐进披露、skill packs、选择性启用），把技能资产分四层归属管理：

| 层 | 内容 | owner | 准入方式 |
|----|------|-------|----------|
| L1 平台通用层 | 全流程阶段技能：specify · design · tdd · review · ship · fix | AI 平台组 | 评测门禁 + 架构委员会评审 |
| L2 领域层 | 领域技能：automotive-development 等，安全红线机器强制 | 架构与领域委员会 | 功能安全负责人会签 |
| L3 语言规范层 | 规范技能：clean-code + java / c / cpp / python 编码规范 | 能力线（原职能线） | PR 评审 |
| L4 团队/项目层 | squad 自有技能 + AGENTS.md 项目级覆盖约束 | 交付小分队 | 向上沉淀须升级评审 |

技能生命周期分三级：草稿（试点 squad 试用）→ 评审入库（团队级可用）→ 生产（组织级，挂评测与回归门禁）。每次晋升必须附该技能的评测报告（见 4.9），不允许"写得像"就直接进生产。

治理纪律五条：变更一律走 PR，当成代码管；技能按命名约定（`<language>-coding-standards`）与 frontmatter description 发现触发，新增技能不改阶段技能；重要知识双形态（模式卡 + 技能红线）；同时启用的技能数量有上限（业界建议控制在 20–50 个），按 skill pack 分组启用，防止上下文污染；失效技能下架，不留僵尸资产。

### 4.9 Skills 工程：评测与自进化

技能自进化靠评测驱动开发（EDD）：每个技能带自己的评测集，用数据证明"带技能比不带好、新版比旧版好"。参照 Anthropic skill-creator 的评测框架（executor / grader / comparator / analyzer 四子代理协作），闭环六步：

1. **评测集**：技能目录内 `evals/evals.json`，每条含任务 prompt、期望输出描述、可验证断言清单。
2. **双跑对比**：每个用例跑两遍——带技能 vs 基线（无技能或旧版本），隔离技能本身的贡献。
3. **盲评打分**：grader 逐条断言判 PASS/FAIL 并附证据引用；A/B 对比时评审方不知道哪边是技能版，消除评估偏置。
4. **出基准**：汇总为 benchmark——通过率、耗时、token 的均值与方差，随技能版本锚定。
5. **诊断改写**：analyzer 找模式——哪些断言有没有技能都能过（无区分度，换掉）、哪些结果方差大（用例不稳定，修），然后改写技能或断言。
6. **回归放行**：达标才允许合入，回到第 1 步迭代，直到没有实质提升。

三道关卡守住质量：**入库关**——新技能必带评测集与基线对比，无评测不入库；**变更关**——技能 PR 自动触发回归评测（CI 集成），通过率不降级才合入；**生产关**——线上一次通过率、知识命中率低于基线自动阻断并告警，接入平台"评估与度量层"（第 2.6 节）。

评测设计四条原则：断言从首轮输出反推（先跑一遍，才知道"好"长什么样）；机械检查用脚本、主观判断用 LLM，断言要有区分度；description 优化按 60/40 train/test 划分防过拟合；高方差用例必须修，否则基准不可信。核心指标四项：触发准确率（description 匹配准不准）、任务通过率（断言 PASS 比例）、一次通过率（生产端 AI 产出质量，入第 7 节度量体系）、成本守卫（token 与时延，防膨胀）。
...
## 附 B：对原开发流程与角色设定的变化对照

本附录用于变革沟通：向设计、开发、测试三个团队说明各自的日常会发生什么变化。

### B.1 开发流程变化对照

| 环节 | 原流程 | 新流程 | 变化性质 |
|------|--------|--------|----------|
| 需求传递 | SE 组织串讲，开发听讲 + 读功能设计说明书 | AR 条目机器可读，AI 答疑；理解对齐由 S1 规格评审兜底 | 串讲消亡 |
| 理解验证 | 无显式环节，理解偏差靠联调暴露 | 新增 AR 规格化：开发主导产出可测试规格，SE 会签 | 新增质量阀门（S1） |
| 实现设计 | 开发手写实现设计说明书（时序图、类图、UT 用例） | AI 基于 AR 规格起草，开发修订；图源码化、随代码进库版本管理 | 写作者变为 AI |
| 编码与 UT | 手写代码和测试代码 | AI 按 TDD 生成（先失败测试后实现），叠加编码规范与车载领域约束，证据落盘 | 生产方式改变 |
| 代码审查 | 评审会 + 人逐行看代码 | AI 独立评审全量初审（作者隔离），人只看 critical 项与设计符合性 | 审查对象从代码行变为 findings |
| 合入 | 审查通过即合入 | DoD 自动核验（追溯更新、测试证据、文档同步）后合入 | 新增机器门禁 |
| 工作粒度 | 按批串讲、按批开发 | 按 AR 逐个流动：规格→设计→代码小步快走 | 批处理变小 |

### B.2 角色变化对照

| 原角色 | 新角色 | 新增职责 | 放下的工作 | 不变的责任 |
|--------|--------|----------|------------|------------|
| SE（设计团队） | 需求工程师，进入小分队 | AI 分解产出的裁决、需求模式库运营、AR 规格会签（S1）、验收会签（G7） | 写长文档、组织串讲 | 需求质量签字（R1/R2） |
| 开发 | 全栈构建工程师 | AR 规格化主导（新质量阀门的 owner）、AI 草稿修订、AI 初审 findings 处置、跨栈工作 | 逐行手写代码与 UT、手写实现设计 | 实现设计签字（R3）、代码终审（R4） |
| 测试 | 质量工程师，进入小分队 | 测试策略与风险地图、AI 生成用例/脚本的评审、缺陷数据标注、探索性测试 | 手工执行回归、手写用例与脚本 | 验收签认（G7） |

### B.3 新增角色与组织归属变化

- 新增三个角色：交付负责人（对本组 SR 从需求到验收端到端负责）、知识/AI 工程师（技能包、知识库、度量面板，可多队共享）、外围的 AI 平台组与架构及领域委员会（功能安全负责人在此收敛全部上抛项）。
- 组织归属：设计/开发/测试三个职能团队的人员进入交付小分队，日常对交付结果负责；职能团队转型为能力线，保留专业标准制定、人才培养与职级评定，跑稳后再考虑进一步扁平化。
- 决策权与开发权分离：开发权交给 AI，决策权（L3 清单：需求优先级、架构关键决策、安全裁决、验收签认等）留在具名的人手里。

### B.4 三个深层转变

1. 串讲消亡的本质不是少开几个会，而是"理解对齐"从口头仪式变成可检查的工件：AR 规格 + SE 会签记录。理解对了没有，从此有证据可查。
2. 质量责任链没有缩短，反而更显化：原有签字点一个不少，还多了 S1。变的是签字人核对的对象从"人写的东西"变成"AI 写的东西"，评审能力成为所有角色的核心技能，这也是初级人员培养的主战场。
3. 人的产出从"工件本身"变成"对工件的判断"：修订痕迹、评审意见、会签结论。这些判断记录不只是过程证据，同时是知识飞轮的原料（见第 4 节），人的经验由此进入组织资产。
