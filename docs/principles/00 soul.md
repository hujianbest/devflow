...
`DevFlow` 不是一套通吃所有研发阶段的 skill family。它只覆盖**开发**阶段；上游的产品 / 架构设计阶段与下游的独立测试阶段由其他 family 承担。

| Family | 阶段 | 主要职责（示意，待对应 family 各自的 soul 文档定义） |
|---|---|---|
| `design-flow` | 产品 / 架构设计 | 产品发现、架构方案、系统级设计；为 devflow 的输入提供已批准的 IR / SR / AR / 组件设计骨架 |
| **`DevFlow`（本 family）** | **软件开发** | 需求规格澄清、组件 / AR 实现设计、TDD 编码、TDD 后测试有效性审查、代码检视、完成门禁、收口 |
| `test-flow` | 独立测试 | 系统 / 集成 / 验收级独立测试设计、执行、回归判断；消费 devflow 的设计与代码证据 |
...
> **devflow 的目标是帮助开发团队把明确输入的需求或问题修改，稳定、高质量、可追溯地落到软件版本中——同时也覆盖把上游需求拆解到 AR 一级的"需求分析 + 可选组件实现设计"阶段。**

这里有三个不能让步的关键词：

- **明确输入**: devflow 不承担产品发现和需求洞察。输入通常已经是 IR / SR / AR、缺陷单、变更请求或团队认可的需求描述；如果输入仍不明确，devflow 只负责澄清规格，不替业务或系统负责人创造需求方向。
- **软件版本**: 实现子街区的一次完成必须覆盖代码、组件 / AR 实现设计、测试证据、TDD 后测试有效性审查和代码检视记录；需求分析子街区的一次完成必须覆盖子系统级需求、可选组件实现设计修订和候选 AR 拆分清单。
- **高质量、可追溯**: devflow 优先保证需求到设计、代码、测试、测试有效性审查、检视的链路可回读、可恢复、可审计，而不是追求"快速改完"。

### 1.1 两个子街区

devflow 在内部划分为两个子街区，各自独立、不可在同一 work item 内跨越：
...
- **实现子街区（AR / DTS / CHANGE）**：
  ...
  - 这是 devflow 之前已经覆盖的传统 DevFlow

devflow 默认使用 `SR<id>-<slug>` 管理需求分析过程目录、`AR<id>-<slug>` 管理需求开发过程目录、`DTS<id>-<slug>` 管理问题修改过程目录。测试设计不作为独立过程文件，而是 AR 实现设计的一部分；SR work item 不含 AR 设计、不含测试设计、不含实现证据。

### 1.2 主场景

devflow 的主场景是既有组件上的增量开发和问题修改；新增组件较少，但必须被明确识别为更高风险路径，触发组件实现设计、架构师确认和更严格的验证要求。SR 子街区是 devflow 自己**内部**承担的需求分析；当上游 design-flow 给出更高层架构判断后，SR 仍可作为 devflow 与下游 AR 实现之间的桥接节点。

### 跨技术栈与跨团队适用性

devflow 不绑定任何具体团队、产品线或技术栈。下面这些是**当前优先支持的典型场景**，不是 devflow 的硬约束：

- 嵌入式 C / C++ 团队：组件设计 / AR 实现设计 / 嵌入式风险（内存 / 并发 / 实时性 / 资源 / 错误处理 / ABI）reviewer rubric 已经覆盖。
- 团队若使用其他语言、其他领域纪律（例如服务端、桌面应用、固件），仍可使用 devflow 的 skill-as-node 契约、handoff 字段、review / gate 角色分离与证据闭环；只需通过 `AGENTS.md` 覆盖具体的模板路径、编码规范、静态分析工具、嵌入式风险维度即可。
- 团队角色名（模块架构师 / 开发负责人 / 开发人员）只是**默认角色画像**；其它团队的等价角色（tech lead / module owner / IC engineer / SRE 等）落到这些画像即可，devflow 不要求改名。

## 二、协作关系：团队专家负责判断，devflow 是工程执行助手

devflow 面向团队使用，不替代团队角色。它的协作关系是：

> **模块架构师 / 开发负责人 / 开发人员负责专业判断，devflow 负责把判断落成可执行、可验证、可追溯的工程过程。**
...
- **devflow**负责按团队模板和质量纪律推进澄清、设计、TDD 实现、TDD 后测试有效性审查、检视和交接，不替任何角色拍板。

## 三、devflow 必须守住的硬性纪律

1. **不替团队定义需求方向。** 输入需求不清时，devflow 做澄清和待决问题列表，不擅自改写 IR / SR / AR 的业务含义。
...
当“快点提交”和“质量证据完整”冲突时，devflow 选择后者。
