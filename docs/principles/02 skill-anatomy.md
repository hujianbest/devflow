...
本文定义 devflow skills 的写作标准，让 `devflow-*` skills 能被 opencode agent 稳定触发、正确执行、留下证据，并能在团队日常开发链路中被恢复编排。

devflow skill 不是一次性提示词，也不是团队流程制度的散文复述。它是一个运行时 contract：告诉 agent 何时加载、处理什么对象、采用什么方法、按什么 todo list 做、产出什么记录、何时必须停止。

## 核心原则

1. **Skill 是团队工程纪律的可执行化。** 团队已有模板、流程、代码规范和验证要求不能只停留在制度里，必须被 skill 转成可执行步骤和证据要求。
2. **devflow 不替团队角色拍板。** 模块架构师、开发负责人和开发人员的判断权不能被 skill 偷偷接管。
...
DevFlow workflow skill 通常是 `Technique + Pattern`。当节点依赖团队模板、测试有效性 rubric、代码检视 rubric 时，再引入 `Reference`。
...
| Router（可选仲裁） | `devflow-router` | 疑难时恢复状态并决定 runtime 下一节点；非 happy-path 必经 |
...
| AR Design | `devflow-ar-design` | AR 代码层设计、测试设计、与组件设计一致性 |
| Implementation | `devflow-tdd-implementation` | 基于 AR 测试设计做 C / C++ TDD |
| Test Review | `devflow-test-review` | TDD 后测试用例有效性、覆盖性、可维护性审查 |
| Code Review | `devflow-code-review` | C / C++ 质量、SOA 边界、嵌入式风险 |
| Gate | `devflow-completion-gate` | evidence bundle 与完成判断 |
| Finalize | `devflow-finalize` | 状态收口、交接、长期记录同步 |
...
- `name` 与目录名一致，使用 `devflow-*` 前缀。
- `description` 只写触发条件和反向边界。
- `description` 不写完整 workflow，不写“先读什么再做什么”的流程摘要。
- 对 opencode 使用场景，description 要前置关键触发词，例如 `spec review`、`AR implementation design`、`component design`、`test review`、`C/C++ code review`。
...
- Boundaries: do not revise component implementation design unless explicitly routed to `devflow-component-design`.
...
- Test Design Before Implementation: define test cases in the AR implementation design before `devflow-tdd-implementation`.
```
...
读最少必要工件（spec / 设计 / 上一节点 record / progress.md / AGENTS.md 路径映射）。把方法落到动作上：例如 Requirements Traceability 要求显式 IR/SR/AR 锚点。证据冲突 → 回 `devflow-router`。
...
同步 `progress.md` canonical 字段，给出唯一 canonical `devflow-*` 下一步；不能映射唯一节点时回 `devflow-router`。
```
...
- 记录哪些 trace links。
- 记录哪些测试、检视或验证证据。
- 下一步 canonical `devflow-*` skill 是什么。
...
- TDD 完成后未经过 `devflow-test-review` 却进入代码检视。
- `devflow-test-review` 试图补写测试或修改生产代码。
...
| 组件设计和 AR 设计边界不清 | 开发人员可能越权改组件架构 | 明确 reroute 到 `devflow-component-design` |
| TDD skill 自己补测试设计 | 跳过 AR 设计职责 | 缺测试设计时回 `devflow-ar-design` |
| TDD 后不做测试有效性审查 | 测试可能只是跑通但覆盖不足 | 路由到 `devflow-test-review` |
| `devflow-test-review` 补写测试或改生产代码 | reviewer 角色越权 | 返回 findings，让实现节点回修 |
