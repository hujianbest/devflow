# DevFlow (devflow) Skill Node Define
...
> 一个 `devflow-*` skill 要成为 DevFlow workflow 中的合法节点，应该如何设计？

在 devflow 中，一个 workflow node 就是一个 skill。Router 编排的不是抽象阶段，而是一组面向嵌入式日常开发的 `devflow-*` skills：它们处理明确需求输入、组件 / AR 设计、C / C++ 实现和代码检视。

devflow 过程目录默认使用 `features/AR<id>-<slug>` 和 `features/DTS<id>-<slug>`。测试设计是 AR 实现设计的一部分，不作为独立 `test-design.md` 文件。

本文不维护完整路由表，也不替代每个 skill 的 `SKILL.md`。它定义 skill-as-node 的设计标准。

## Core Definition

一个 devflow skill node 是一个满足以下条件的 `devflow-*` skill：
...
9. 能被 `devflow-router` 根据磁盘证据重新进入。

```text
devflow skill node = skill + responsibility + object + methods + workflow todos + artifacts + evidence + handoff
```

## Five Contracts

每个 devflow node skill 必须满足五类 contract。
...
| Component Design Review | `devflow-component-design-review` | 独立评审组件实现设计                            | 顺手改设计                |
...
| Hotfix / Problem Fix    | `devflow-problem-fix`             | 处理缺陷复现、根因、最小安全修复和回流                   | 绕过设计与验证              |


## Object Contract

devflow skill node 必须明确它处理的对象。


| Node                     | Frontend Input Object   | Primary Object                        | Backend Output Object                  | Transformation          |
| ------------------------ | ----------------------- | ------------------------------------- | -------------------------------------- | ----------------------- |
| `devflow-specify`            | 明确输入需求、IR/SR/AR、缺陷单     | requirement specification model       | 澄清后的需求规格、待决问题、AR 边界                    | 把已有需求输入澄清为可设计对象         |
| `devflow-spec-review`        | 需求规格、IR/SR/AR 追溯、待决问题   | spec finding set                      | spec review record、verdict、next action | 把规格对象审查成发现项和迁移结论        |
...
devflow 节点必须声明采用的方法，并说明方法如何支撑对象转换。
...
每个 devflow skill 的 `Workflow` 必须写成可执行 todo list。每一步用一个二级 / 三级标题概括动作，正文说明：**遵循什么方法**、**读什么 / 写什么**、**什么时候停下**。不堆无关检查、不强制每步都列 Object / Input / Output / Stop 字段。
...
devflow skill node 必须定义 hard gates、red flags 和 verification。
...
- `next_action_or_recommended_skill` 必须使用 canonical `devflow-*` 节点名。
- 设计、检视、测试结论必须落盘。
- 下一步不唯一时，必须回到 `devflow-router`。

## Design Checklist For New devflow Nodes

新增或重写一个 `devflow-*` skill 前，至少检查：
