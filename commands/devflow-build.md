...
- Skills involved (in推进顺序):
  ...
  2. 本命令（编排者）派发测试有效性评审（`devflow-test-review`）
  3. test-review 通过后，本命令（编排者）**顺序**派发 C/C++ 代码检视（`devflow-code-review`）
- happy path 由各 skill 的 `Entry Gate` / `Exit Handoff` + 证据自路由（读 `features/<id>/progress.md`）驱动，疑难才调用可选的 `devflow-router` 仲裁
- Reviewers dispatched (由本命令（编排者）**顺序**派发, system prompt = `agents/devflow-reviewer.md`):
  - `devflow-test-review`（`target_skill = devflow-test-review`）
  - `devflow-code-review`（`target_skill = devflow-code-review`，**仅在 test-review 通过后**，不并行）
...
- 多个 `in_progress` task 或 next-ready 不唯一 → `reroute = true`，停下交可选的 `devflow-router` 仲裁（`references/devflow-conventions.md` §9 Hard Stop #8）
- TDD 完成后 **未经** `devflow-test-review` → 不得进入 `devflow-code-review`（§9 Hard Stop #9；评审链顺序、不并行 fan-out）
...
1. 读 `features/<id>/progress.md`、`Task Plan Path`、`Task Board Path`：
   - 工件证据不稳定 / next-ready 不唯一 → 回 `/devflow`（`using-devflow` 发现 / 证据自路由；疑难交可选的 `devflow-router` 仲裁）
...
4. 子代理返回：
   ...
   - `BLOCKED` + `reroute = true` → 交可选的 `devflow-router` 仲裁
5. 所有 next-ready task 完成 → 依其 `Exit Handoff`，由本命令（编排者）派发独立 `devflow-reviewer` 子代理执行 `devflow-test-review`
6. 消费 test-review verdict：
   - `APPROVE` / `APPROVE_WITH_FOLLOWUPS` → 步骤 7
   - `REQUEST_CHANGES` → 回 `devflow-tdd-implementation` 修测试 / 修实现
   - `REJECT` → 停下，交开发负责人裁决
7. test-review 通过后，由本命令（编排者）**顺序**派发独立 `devflow-reviewer` 子代理执行 `devflow-code-review`（不与 test-review 并行）
...
| "code-review 我自己读一遍代码就行" | 禁止；必须由本命令（编排者）派发独立子代理 |
