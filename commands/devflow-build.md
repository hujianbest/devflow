...
1. 前置检查：先从 plan.md 或 `using-devflow` 解析目标组件根与工件根；该组件根下 `features/<id>/design.md`（或团队覆盖路径）存在、R2 评审门禁通过（plan.md 门禁表 + reviews/ 记录可核）且含测试设计表；否则回 `/devflow-design` 或 `/devflow-review`。
...
3. 先按 `plan-template.md` 细化同一组件根/工件根下 `features/<id>/plan.md`（或团队覆盖路径）任务拆解（自包含任务：用例锚点、精确路径、步骤、完成定义），再逐任务执行；任务状态、步骤勾选与 RED/GREEN 证据行实时写回 plan.md，任务完成时更新 `traceability.md` 对应列；一次一个任务，每任务完成即提交。中断恢复从 plan.md 第一个未完成任务继续。
