...
- Craft 透镜（评审子代理内部叠加，不是流程节点、不进 handoff、不产 verdict）：
  - `devflow-test-review` 以 `devflow-test-craft`、`devflow-code-review` 以 `devflow-coding-craft`、`devflow-component-design-review` / `devflow-ar-design-review` 以 `devflow-design-craft` 为判别标尺
