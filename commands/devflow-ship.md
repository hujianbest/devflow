...
- Skills involved (in推进顺序):
  ...
  2. `devflow-finalize` — 实现闭环：promote `docs/ar-specs/`、`docs/ar-designs/`、可选长期资产
- Reviewers dispatched: 无（评审在 `/devflow-build` 阶段已经完成；本阶段只做门禁与闭环）

## When to use

- `devflow-code-review` 已 `APPROVE`，准备过完成门禁
- 门禁返工后复检
- 用户明确要求收口某个 AR
...
- `devflow-completion-gate` 未通过 → **不得** 进入 `devflow-finalize`
...
1. 读 `features/<id>/progress.md`、`reviews/`、`evidence/`、`completion.md`（如有）
   - `devflow-code-review` 未通过 → 回 `/devflow-build`
   - 工件证据冲突 → 回 `/devflow` 走 router
2. 进入 `devflow-completion-gate`，严格遵循其 SKILL.md `工作流`，输出 `PASS` / `HOLD` / `FAIL`
   - `PASS` → 进入 `devflow-finalize`
   - `HOLD` → 写 `Blockers`，下一步指向 SKILL.md 指定的回修节点（通常是 `devflow-tdd-implementation` 或某个评审节点），由 `/devflow-build` 继续
   - `FAIL` → 停下，交开发负责人裁决
3. 在 `devflow-finalize` 内：
...
4. 输出 canonical handoff 块；闭环完成后 `Current Stage = devflow-finalize`，`Pending Reviews And Gates` 清空，`Blockers` 清空或留 follow-up
