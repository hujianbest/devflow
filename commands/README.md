...
DevFlow 工作流阶段入口（slash-style commands）。每个 command 是 **平台适配层表面**，方便用户按阶段表达意图；内部组合多个 canonical `devflow-*` skill，但不替换或绕开它们。
...
| Persona（agent） | 带角色视角和输出格式的子代理 | `devflow-reviewer`、`devflow-implementer` | *Who*：可被独立调用（常由 router / tdd-implementation 派发，也可直接发起）|
| Command | 用户视角的工作流阶段入口 | `/devflow-specify`、`/devflow-build` | *When*：组合多个 skill 推进一个阶段 |

Slash command **不复制** SKILL.md 的内容，只声明本阶段会跨过哪些 canonical 节点、需要派发哪些评审子代理，以及阶段开始与结束时如何形成 handoff。所有判据仍以 `skills/<name>/SKILL.md` 为唯一权威；编码规范与领域约束 skills 只作为支持约束，不作为 command 或 runtime next action。
...
2. **不内联自审**。所有 5 个评审节点（spec / component-design / ar-design / test / code）必须由 **独立** `devflow-reviewer` 子代理执行；调用方不限（随流程通常由 `devflow-router` 派发，按需评审由 `/devflow-review` 发起，也可由用户直接发起）。核心约束只有一条：禁止父会话或作者节点自审。
...
- ❌ `/code-simplify`：简化属于 `devflow-clean-code` 与适用编码规范，在 `devflow-tdd-implementation` 的 REFACTOR 步骤内部消费，不另立命令或节点。
- ℹ️ `devflow-clean-design`、`devflow-clean-code`、编码规范 skills（如 `c-coding-standards` / `cpp-coding-standards`）和领域约束 skills（如 `embedded-development` / `automotive-development`）不设独立 command：它们由设计 / 构建 / 评审 / 门禁节点在工作流内部消费，提升产物质量，不改变流程拓扑、不进 handoff、不产 verdict。
- ❌ `/plan`：tasks 队列由 `devflow-tdd-implementation` 内部 preflight 管理，不另立 command。
- ❌ 任何 meta-orchestrator command：运行时证据路由归 `devflow-router`，不另立与之竞争的编排命令（各 command / agent 本身仍可被独立调用，这里只是不再叠一层编排器）。
